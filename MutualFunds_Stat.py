# -*- coding: utf-8 -*-
"""
Created on Mon Jul 27 21:00:20 2020
...
def meanRetAn(data):             
    ...
    for portfolio in range(N_portfolio):
        if profit[portfolio] < 0:
            Result[portfolio] = -((1+abs(profit[portfolio]))**(1/float(4.08))-1)
        else:
...
# Functions computing the Sharpe Ratio    
...
# Functions compution the Sortino ratio    
def sortinoAn(data):
    N_portfolio = len(data[:,0])
    Result = np.empty(N_portfolio, dtype=float)
    mu_ga = meanRetAn(data)    
...
# Function computing the number of months when the strategy beats benchmark
def beatMonth(data):
    N_portfolio = len(data[:,0])
    Result = np.empty(N_portfolio, dtype=float)
...
# Function computing the number of years when the strategy beats benchmark
def beatYear(data):
    N_portfolio = len(data[:,0])
    Result = np.empty(N_portfolio, dtype=float)
...
#################################################
nameExcel = "out-MutualFundRet"
nameSheet = "MutualPortVal"
firstRun = "True"                                  #True of False


# Read our data from Excel sheet
excelSheet = xlrd.open_workbook("C:/Users/Petr/Documents/Python Scripts/Excels/" + nameExcel +".xlsx")
